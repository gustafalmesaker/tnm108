...
def group_by_class(self, data, target):
 ...
 #build model
"""
The probability of each group of instances (that is the class) with respect to the total number of instances
"""
#len(group)/len(data)
...
def joint_probabilities(self, data):
   ...
   :return: Use the normal_pdf(self, x, mean, stdev) to calculate the Normal Probability for each feature
...
def posterior_probabilities(self, test_row): 
    ...
    E.g.
    prior_prob: P(setosa)
    likelihood: P(sepal length | setosa) * P(sepal width | setosa) * P(petal length | setosa) * P(petal width | setosa) joint_prob: prior_prob * likelihood
